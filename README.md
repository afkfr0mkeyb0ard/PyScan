...
### Command
```
> python3 pyscan.py target ports [-o] [-t 10] [-d 0] [-v]
...
-d          delay between requests in seconds (default 0), and can be randomized with min/max value: 1-4, 11-15, ...
-v          verbose to display closed ports (default False)
-h          display help
```
...
# Scan for open ports 135,139,3389 on a list of IP with 3 seconds delay
> python3 pyscan.py IPS.txt 135,139,3389 -d 3
...
# Scan for open port 135,139,3389 on a single IP with random delay between 5 and 20 seconds
> python3 pyscan.py 192.168.0.123 135,139,3389 -d 5-20
```
